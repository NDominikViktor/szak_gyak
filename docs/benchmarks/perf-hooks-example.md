...
## Titkosítási idő mérése (`perf_hooks`)
...
**<a id="1-tablazat"></a>1. táblázat:** az első körös (kibővítés előtti)
titkosítási mérés eredménye.
...
## HTTP szerver terheléses tesztje (`autocannon`)
...
**<a id="2-tablazat"></a>2. táblázat:** `autocannon` terheléses teszt,
két egymást követő futtatás.
...
Az [1. ábra](#1-abra) mutatja a pipeline teljes adatfolyamát (mérés →
CSV → ábra → dokumentum-build, `scripts/run-pipeline.mjs`):
...
**<a id="1-abra"></a>1. ábra:** a mérés-ábra-dokumentum pipeline
adatfolyama: melyik szkript milyen fájlt ír/olvas, és hogyan jut el a
végeredmény a GitHub Pages-ig.
...
kiegészítés, a fő mérési eredmények ([1. táblázat](#1-tablazat) és
[2. táblázat](#2-tablazat)) már megvannak.
